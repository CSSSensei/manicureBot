    ...
    if added_slots:
        ...
        for date in sorted_dates:
            ...
            for start, end in time_slots:
                result_text += PHRASES_RU.replace(
    ...
    if not_added_slots:
        ...
        for date in sorted_dates:
            ...
            for error, start, end in time_slots:
                result_text += PHRASES_RU.replace(
                    'template.master.slot_time_range_with_error',
...
async def backup_db(bot: Bot):
    try:
        ...
        integrity_check = subprocess.run(
            ['sqlite3', str(db_path), 'PRAGMA integrity_check;'], capture_output=True, text=True
        )
        ...
        await bot.send_document(
            const.ADMIN_ID, document=FSInputFile(f'{backup_path}.gz'), caption='🔧 Бэкап БД', disable_notification=True
        )

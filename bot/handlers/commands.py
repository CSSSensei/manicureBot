...
async def _(message: Message):
    await message.answer(PHRASES_RU.commands.start, reply_markup=default.keyboard)


@router.command('help', 'как пользоваться ботом')  # /help
async def _(message: Message):
    await message.answer(PHRASES_RU.commands.help, reply_markup=default.keyboard)


@router.command('about', 'о разработчиках')  # /about
async def _(message: Message):
    await message.answer(PHRASES_RU.commands.about, disable_web_page_preview=True, reply_markup=default.keyboard)
...
async def _(message: Message):
    ...
    await message.answer(PHRASES_RU.title.commands + commands_text, reply_markup=default.keyboard)
